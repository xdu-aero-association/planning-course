...
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 &pointcloud_map) {
  ...
  for (int idx = 0; idx < (int)cloud.points.size(); idx++) {
    ...
    // for visualize only
    Vector3d cor_round =
        _RRTstar_preparatory->coordRounding(Vector3d(pt.x, pt.y, pt.z));
    pt.x = cor_round(0);
    pt.y = cor_round(1);
    pt.z = cor_round(2);
...
getPathLengthObjective(const ob::SpaceInformationPtr &si) {
  return ob::OptimizationObjectivePtr(
      new ob::PathLengthOptimizationObjective(si));
}

ob::OptimizationObjectivePtr
getThresholdPathLengthObj(const ob::SpaceInformationPtr &si) {
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt) {
  ...
  si->setStateValidityChecker(
      ob::StateValidityCheckerPtr(new ValidityChecker(si)));
  si->setup();
  ...
  *
  *
  STEP 5: Set the optimization objective, the options you can choose are defined
  earlier:
...
int main(int argc, char **argv) {
  ...
  _RRTstar_path_vis_pub =
      nh.advertise<visualization_msgs::Marker>("RRTstar_path_vis", 1);
  ...
  _RRTstar_preparatory->initGridMap(_resolution, _map_lower, _map_upper,
